...
def hero_makes_move(hero, matrix, animals, round_):
    clear()
    print_matrix(matrix)
    print("--= Головне меню =--")
    ...
    if choice in ['w', 'a', 's', 'd']:
        ...
        choice = press()
        creatures_rolls(hero, choice, round_)
    elif choice == "t":
        plant_trap(matrix,hero,trap,round_)
...
def creatures_rolls(creature, direction, round_):
    if round_ % 5==0:
        if direction == 'w':
            if creature['coords'][0] > 1:
                creature['coords'][0] -= 2
        elif direction == 'a':
            if creature['coords'][1] == 0:
                creature['coords'][1] = M - 2
            else:
                creature['coords'][1] -= 2
        elif direction == 's':
...
def is_kurka_catched(animals, hero):
    catched_animals = []
    if hero["coords"]==trap["coords"]:
        hero["hp"]-=20

    for animal in animals:
        if animal['coords'] == hero['coords'] or animal['coords'] == trap['coords']:
...
def plant_trap(matrix,hero,trap,round_,):
    if round_ % 10 == 0 :
        direction=input("1,2,3,4-->")
        trap['coords'][0]=hero['coords'][0]
        trap['coords'][1]=hero['coords'][1]
        if direction=='1':
            trap['coords'][0]-=1
        elif direction=='2':
            trap['coords'][1]+=1
            ...
            trap['coords'][1]-=1
        matrix[trap['coords'][0]][trap['coords'][1]] = trap['face']
        animals.append(trap)
...
def main(animals, hero):
    ...
    while True:
        ...
        hero_makes_move(hero, matrix, animals, round_)
